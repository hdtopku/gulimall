    ...
    <el-dialog title="合并到整单" :visible.sync="mergedialogVisible">
      ...
      <el-select v-model="purchaseId" placeholder="请选择" clearable filterable>
        <el-option
          ...
          :key="item.id"
          :label="item.id+':'+item.assigneeName+item.phone"
          :value="item.id"
        >
...
export default {
  ...
  methods: {
    mergeItem () {
      ...
      if (!this.purchaseId) {
        this.$confirm(
          ...
          .then(() => {
            this.$http({
              url: this.$http.adornUrl('/wms/purchase/merge'),
              method: 'post',
    ...
    getUnreceivedPurchase () {
      this.$http({
        url: this.$http.adornUrl('/wms/purchase/unreceive/list'),
        method: 'get',
    ...
    getWares () {
      this.$http({
        url: this.$http.adornUrl('/wms/wareinfo/list'),
        method: 'get',
        params: this.$http.adornParams({
          page: 1,
          limit: 500
