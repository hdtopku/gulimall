        ...
        <el-card class="box-card" style="width: 80%; margin: 20px auto">
          ...
          >
            ...
            <el-form-item label="商品介绍" prop="decript">
              ...
              <el-input @keyup.enter.native="addDescriptUrl" clearable v-model="descriptUrl" placeholder="也可粘贴图片链接，回车后添加"></el-input>
            </el-form-item>

            <el-form-item label="商品图集" prop="images">
              <multi-upload v-model="spu.images"></multi-upload>
              <el-input @keyup.enter.native="addImagesUrl" clearable v-model="imagesUrl" placeholder="也可粘贴图片链接，回车后添加"></el-input>
            </el-form-item>
            <el-form-item>
  ...
  methods: {
    addImagesUrl () {
      if (this.imagesUrl != null && this.imagesUrl !== '') {
        this.spu.images.push(this.imagesUrl)
      }
      this.imagesUrl = null
    },
    addDescriptUrl () {
      if (this.descriptUrl != null && this.descriptUrl !== '') {
        this.spu.decript.push(this.descriptUrl)
      }
      ...
      if (!this.dataResp.steped[1]) {
        this.$http({
          url: this.$http.adornUrl(
            `/pms/attr/sale/list/${this.spu.catalogId}`
          ),
          method: 'get',
          params: this.$http.adornParams({
            page: 1,
    ...
    showBaseAttrs () {
      if (!this.dataResp.steped[0]) {
        this.$http({
          url: this.$http.adornUrl(
            `/pms/attrgroup/${this.spu.catalogId}/withattr`
          ),
          method: 'get',
          params: this.$http.adornParams({})
        })
    ...
    submitSkus () {
      ...
      this.$confirm('将要提交商品数据，需要一小段时间，是否继续?', '提示', {
        ...
        .then(() => {
          this.$http({
            url: this.$http.adornUrl('/pms/spuinfo/save'),
            method: 'post',
